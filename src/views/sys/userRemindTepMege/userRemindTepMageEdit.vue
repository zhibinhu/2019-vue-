        ...
        methods: {
            ...
            saveUserRemindTep () {
                ...
                this.$myHttp({
                    ...
                    data: {'entity': {'id': this.$route.params.id, 'englishName': _this.ruleForm.englishName}}
                }).then(resEn => {
                    if (resEn.data.rows.length != '0') {
                        ...
                        this.$myHttp({
                            ...
                            data: {'entity': {'id': this.$route.params.id, 'chineseName': _this.ruleForm.chineseName}}
                        }).then(resCn => {
                            if (resCn.data.rows.length != '0') {
                                ...
                                this.$myHttp({
                                    ...
                                    data: {'entity': {'id': this.$route.params.id, 'englishName': _this.ruleForm.englishName, 'chineseName': _this.ruleForm.chineseName, 'templateType': _this.ruleForm.templateType, 'content': _this.msg, 'contentReplace': '', 'contentReplaceList': _this.contentReplaceList}}
                                }).then(res => {
                                    _this.$router.push({
            ...
            backfillData () {
                ...
                if (this.$route.params.id) {
                    sessionStorage.setItem('userRemindTepMage_dataId', this.$route.params.id);
                }
                var getDataId = sessionStorage.getItem('userRemindTepMage_dataId');
                var dataId = this.$route.params.id ? this.$route.params.id : getDataId;
                if (dataId) {
                    this.$myHttp({
                        method: 'POST',
                        url: this.permissionPrefix + '/microarch/sys/sysUserTemplate/view',
                        data: {
                            'entity': {
                                'id': dataId
                            }
                        ...
                        console.log('sysUserTemplateData---', res);
                        if (res.data.entity) {
                            _this.ruleForm = res.data.entity;
                            _this.msg = res.data.entity.content;
                            for (var i = 0; i < res.data.entity.contentReplaceList.length; i = i + 2) {
                                this.tableData.push({
                                    'ekey': res.data.entity.contentReplaceList[i],
                                    'evalue': res.data.entity.contentReplaceList[i + 1]
                                });
                            }
                        }
                    });
